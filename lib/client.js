...
function validateKey(key, operation) {
  ...
  misc.assert(Buffer.byteLength(key) < 250, "Key must be less than 250 bytes long");
  misc.assert(key.length < 250, "Key must be less than 250 bytes long");
...
Client.prototype.getHostList = function () {
  ...
  return Promise.any(
    this.hosts.map(function (host) {
      ...
      const ConnClass = this.poolSize > 1 ? ConnectionPool : Connection;
      connections[host] = new ConnClass({
        host: h.host,
        port: h.port,
...
Client.prototype.connectToHosts = function () {
  ...
  this.hosts.forEach(function (host) {
    ...
    // Connect to host
    const ConnClass = this.poolSize > 1 ? ConnectionPool : Connection;
    this.connections[host] = new ConnClass({
      host: h.host,
      port: h.port,
