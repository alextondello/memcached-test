...
Connection.prototype.read = function (data) {
  debug('got data: "%s" and the queue now has "%d" elements', misc.truncateIfNecessary(data), this.commandBuffer.size);
  ...
  if (deferred.type === "autodiscovery") {
    ...
    if (data.match(/^CONFIG .+/)) {
      ...
      this.autodiscover.len !== undefined &&
      this.autodiscover.version === undefined &&
      data.match(/^([0-9])+/)
    ) {
...
Connection.prototype.write = function (str) {
  ...
  if (this.ready && this.writeBuffer.size < 1) {
    ...
    this.commandBuffer
      .first()
      .reject("Error, Connection to memcache lost and buffer over " + this.bufferBeforeError + " items");
    this.commandBuffer = this.commandBuffer.shift();
  }
};
...
Connection.prototype.gets = function (key, opts) {
  ...
  return deferred.promise.timeout(this.netTimeout).spread(function (data, flag, length, cas) {
    if (data) {
...
Connection.prototype.get = function (key, opts) {
  ...
  return deferred.promise.timeout(this.netTimeout).spread(function (data, flag, length) {
    if (data) {
      if (opts.compressed || flag & FLAG_COMPRESSED) {
        // @todo compressed data should still be able to utilize the
        //  flags to return data of same type set
...
Connection.prototype.flush_all = function (delay) {
  ...
  return deferred.promise.then(function (v) {
    return v === "OK";
  });
};
    ...
    .then(function (v) {
      ...
      return deferred.promise.then(function (data) {
        ...
        if (data === "NOT_STORED") {
          ...
          throw new Error(
            util.format("Something went wrong with the replace. Expected STORED, got :%s:", data.toString())
          );
        } else {
          return Promise.resolve();
        }
    ...
    .then(function (v) {
      ...
      return deferred.promise.then(function (data) {
        ...
        if (data === "NOT_STORED") {
          ...
          throw new Error(
            util.format("Something went wrong with the append. Expected STORED, got :%s:", data.toString())
          );
        } else {
          return Promise.resolve();
        }
    ...
    .then(function (v) {
      ...
      return deferred.promise.then(function (data) {
        ...
        if (data === "NOT_STORED") {
          ...
          throw new Error(
            util.format("Something went wrong with the prepend. Expected STORED, got :%s:", data.toString())
          );
        } else {
          return Promise.resolve();
        }
